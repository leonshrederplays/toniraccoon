...
message.delete();
    ...
    if (url.match(pl)) {
        ...
        return message.channel.send(`🎵 **${playlist.title}** has been added to queue.`).then(msg => msg.delete(5000));
    } else {
        ...
        try {
            ...
            try {
                ...
                let embed = new discord.RichEmbed()
                    ...
                    .setDescription(`**-=- Musik Suchergebnisse -=-**\n${videos.map(video => 
                        `**${++index} -** ${video.title}`).join('\n')}\n\n🎵 Wähle einen Titel zwischen **1** und **10** in den nächsten **20 sekunden**.`);
                ...
                try {
                    ...
                    if (err) undefined
                    return message.channel.send(utils.cmd_fail('⚠ Die 20 sekündige auswahl Zeit ist abgelaufen.', `${config.prefix}search <music>`)).then(msg => msg.delete(5000));
                }
                const videoIndex = parseInt(response.first().content);
                var video = await bot.youtube.getVideoByID(videos[videoIndex - 1].id);
            } catch (err) {
                console.error(err);
                return [message.delete(), utils.timed_msg(utils.cmd_fail(`${message.author}, es kann kein Video mit diesen worten gefunden werden: \`${searchString}\``, `${config.prefix}play <music/url>`), 5000)];
            }
        }
